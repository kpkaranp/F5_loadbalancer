...
def main():
    ...
    if not username or not password:
        raise ValueError("F5_USERNAME and F5_PASSWORD environment variables must be set")
    ...
    for device in devices:
        ...
        safe_host = str(host).replace("https://", "").replace("http://", "").replace("/", "_")
        output_file = os.path.join(
            args.output_dir,
            f"{safe_host}_{timestamp}_f5_list.csv"
        )
