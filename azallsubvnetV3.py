...
def main():
    ...
    for sub_id in subscriptions:
        print(f"Working on subscription: {sub_id}")
        network_client = NetworkManagementClient(credential, sub_id)

        try:
            vnets = list(network_client.virtual_networks.list_all())
        except Exception as e:
            print(f"Could not fetch VNets for subscription {sub_id}: {e}")
            continue
        ...
        for vnet in vnets:
            if "hub" in vnet.name.lower():
                ...
                print(f"Found HUB VNet: {vnet_name} in {vnet_rg}")

                try:
                    peerings = list(network_client.virtual_network_peerings.list(vnet_rg, vnet_name))
                except Exception as e:
                    print(f"Error fetching peerings for {vnet_name}: {e}")
                    continue
                ...
                for peer in peerings:
                    ...
                    try:
                        ...
                        print(f"Error parsing peer VNet ID {peer_vnet_id}: {e}")
                        peer_subscription_id = peer_resource_group = peer_vnet_name = "Unavailable"

                    # Try to get the peered VNet's address space
                    try:
                        remote_vnet = network_client.virtual_networks.get(peer_resource_group, peer_vnet_name)
                        address_space = ", ".join(remote_vnet.address_space.address_prefixes)
                    except Exception as e:
                        print(f"Error fetching address space for remote VNet {peer_vnet_name}: {e}")
                        address_space = "Unavailable"
                    ...
                    all_data.append({
                        ...
                        "Peering IP Address Space": address_space
                    })
